...
contract BooksLibrary {
    ...
    function addBook(
        ...
        require(
            bytes(books[_index].title).length == 0,
    ...
    function addMyBook(
        ...
        require(
            bytes(myBooks[msg.sender][_id].title).length == 0,
            "Book already exists at this ID for the user"
        );
    ...
    function getMyBooks(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(
                bytes(myBooks[msg.sender][_ids[i]].title).length != 0,
